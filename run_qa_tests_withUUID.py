...
# Force using Vertex AI / Agent Platform for the google-genai SDK
...
def get_est_timestamp() -> str:
    ...
    tz_name = now_est.strftime("%Z")
    return now_est.strftime(f"%Y/%m/%d %H:%M:%S {tz_name}")
...
def format_test_case_prompt(row, project_id: str, region_id: str, app_id: str, conversation_id: str) -> str:
    ...
    # Inject conversationId into required_vars
    if required_vars:
        if "conversationid" not in required_vars.lower():
            lines = [line.strip() for line in required_vars.split('\n') if line.strip()]
            lines.append(f"- conversationId = {conversation_id}")
            required_vars = "\n        ".join(lines)
    else:
        required_vars = f"- conversationId = {conversation_id}"
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="content-header">
            ...
            <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
                <button class="btn-pdf" data-html2pdf-ignore onclick="exportToPDF()" style="display: flex; align-items: center; gap: 8px; font-weight: 600; padding: 10px 16px; border-radius: 8px; background-color: var(--primary); color: white; border: none; cursor: pointer; transition: var(--transition); font-family: 'Inter', sans-serif; font-size: 0.9rem; box-shadow: 0 4px 10px rgba(196, 18, 48, 0.2);">
        ...
        <div id="section-dashboard" class="section-view active">
            ...
            <div class="dashboard-card">
                ...
                <div class="card-body">
                    ...
                    <div class="filters-row" data-html2pdf-ignore>
                        ...
                        <table id="resultsTable">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 130px; text-align: right;" data-html2pdf-ignore>Action</th>
                                </tr>
                            </thead>
        ...
        <div id="section-detailed" class="section-view">
            ...
            <!-- Selector Row -->
            <div class="detailed-selector-wrapper" data-html2pdf-ignore>
            ...
            <div id="detailed-report-content">
                ...
                <div class="report-grid-meta">
                    ...
                    <div class="meta-item">
                        ...
                        <span class="meta-val" id="meta-conversation-id" style="font-family: monospace; font-size: 0.8rem;">--</span>
                    </div>
                </div>
    ...
    <script>
        ...
        function exportToPDF() {
            ...
            // Determine filename
            let filename = 'QA_Automation_Dashboard.pdf';
            if (activeTab === 'detailed') {
                const tcid = document.getElementById('meta-tcid').innerText;
                filename = `QA_Test_Report_TCID_${tcid}.pdf`;
            }
        ...
        function renderResultsTable() {
            ...
            filteredData.forEach(tc => {
                ...
                tr.innerHTML = `
                    ...
                    <td style="text-align: right;" data-html2pdf-ignore>
        ...
        function populateTestCaseSelector() {
            ...
            testCasesData.forEach(tc => {
                ...
                option.innerText = `[TC-${tc.tcid}] ${statusSymbol} ${tc.summary || ('Test Case ' + tc.tcid)}`;
        ...
        function loadTestCaseReport(tcid) {
            ...
            document.getElementById('meta-conversation-id').innerText = tc.conversationId || '--';

            // Objective & Scope
            const csvSummary = tc.summary || 'No summary description available.';
            const objective = `
                <div style="font-size: 0.95rem; line-height: 1.6; color: var(--text-secondary);">
                    <strong>Test Case Objective:</strong> ${csvSummary}
                </div>
            `;
            document.getElementById('report-objective').innerHTML = objective;
...
def generate_report(results_dir: str, output_html_path: str):
    print(f"Reading QA results from: {results_dir}")
    if not os.path.exists(results_dir):
        print(f"Error: Results path not found at '{results_dir}'.")
        return False
    ...
    try:
        if os.path.isdir(results_dir):
            for filename in os.listdir(results_dir):
                if filename.startswith("tcid_") and filename.endswith(".json"):
                    filepath = os.path.join(results_dir, filename)
                    try:
                        with open(filepath, "r", encoding="utf-8") as f:
                            test_cases.append(json.load(f))
                    except Exception as je:
                        print(f"Warning: Skipping malformed or unreadable JSON file {filename}: {je}")
        else:
            # Fallback for compatibility with single JSONL file
    ...
    # Calculate execution time in EST
    try:
        from zoneinfo import ZoneInfo
        tz = ZoneInfo("America/New_York")
    except ImportError:
    ...
    now_est = datetime.now(tz)
    tz_name = now_est.strftime("%Z")
    est_timestamp = now_est.strftime(f"%Y/%m/%d %H:%M:%S {tz_name}")
    
    # Replace the placeholders in the template
    report_content = HTML_TEMPLATE.replace("##TEST_CASES_DATA##", test_cases_json)
    report_content = report_content.replace("##EXECUTION_TIMESTAMP##", est_timestamp)
...
def build_run_output_paths(input_csv_path: str) -> tuple[str, str]:
    """Derive output paths from the input CSV name inside a timestamped output folder."""
    timestamp_folder = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_output_dir = os.path.join("output", timestamp_folder)

    os.makedirs(run_output_dir, exist_ok=True)

    report_file_path = os.path.join(run_output_dir, "report.html")

    return report_file_path, run_output_dir
...
async def main():
    ...
    # Propagate the configuration to environment variables so that downstream libraries
    # (like google-genai and google.adk) use the CLI-specified values instead of .env placeholders.
    # os.environ["GOOGLE_CLOUD_PROJECT"] = gcp_project
    os.environ["GCP_PROJECT_ID"] = gcp_project
    os.environ["GCP_REGION_ID"] = gcp_region
    os.environ["GCP_APP_ID"] = gcp_app

    input_file_path = os.path.abspath(args.input_file)
    html_output_path, run_output_dir = build_run_output_paths(input_file_path)
    ...
    async with runner:
        ...
        for idx, row in enumerate(test_cases, start=1):
            ...
            # Generate unique conversationId for the test case
            ...
            prompt = format_test_case_prompt(row, gcp_project, gcp_region, gcp_app, conversation_id)
            ...
            try:
                ...
                if "timestamp" not in result_json or not result_json["timestamp"]:
                    result_json["timestamp"] = get_est_timestamp()
                ...
                if "conversationId" not in result_json:
                    result_json["conversationId"] = conversation_id
                
                # Incremental save: Write individual JSON file
                status = str(result_json.get("result", "failed")).strip().lower()
                if status not in ["passed", "failed"]:
                    status = "failed"
                tcid_filename = f"tcid_{tcid}_{status}.json"
                tcid_filepath = os.path.join(run_output_dir, tcid_filename)
                with open(tcid_filepath, "w", encoding="utf-8") as f:
                ...
                try:
                    tcid_filename = f"tcid_{tcid}_failed.json"
                    tcid_filepath = os.path.join(run_output_dir, tcid_filename)
                    with open(tcid_filepath, "w", encoding="utf-8") as f:
            ...
            # Add a small delay between test cases to prevent hitting Vertex AI rate limits (429 Resource Exhausted)
            if idx < total_cases:
                import asyncio
                # A 5-second sleep paces the requests to ~12 RPM, which fits within standard project trial quotas.
    ...
    if args.generate_report:
        try:
            ...
            generate_report(run_output_dir, html_output_path)
        except Exception as e:
